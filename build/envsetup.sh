...
cat <<EOF
Additional CandyRoms functions:
...
- candyremote:      Add a git remote for CANDY github repository.
...
function candygerrit() {
    ...
    local user=`git config --get https://gerrit.bbqdroid.org.username`
...
function candyrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CandyRoms Gerrit Rebase Usage: "
    ...
    git fetch "https://gerrit.bbqdroid.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
